...
export default function LearnerDashboard() {
    ...
    const weeklyData = stats?.weeklyActivity?.map((item: any) => ({
        date: format(parseISO(item.date), 'EEE'),
        completions: parseInt(item.count)
    })) || [];

    // Prepare data for Learning Time Chart
    const timeData = stats?.learningTimeTrend?.map((item: any) => ({
            ...
            <main className="px-4 py-6 max-w-3xl mx-auto space-y-8">
                ...
                <section className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in">
                    <div className="bg-card rounded-2xl border border-border/50 p-5 shadow-soft">
                        ...
                        <p className="text-2xl font-bold text-foreground">{isLoading ? '-' : stats?.streak || 0}</p>
                    ...
                    <div className="bg-card rounded-2xl border border-border/50 p-5 shadow-soft">
                        ...
                        <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Learning Time</p>
                    </div>
                    ...
                    <div className="bg-card rounded-2xl border border-border/50 p-5 shadow-soft">
                        ...
                        <p className="text-2xl font-bold text-foreground">{isLoading ? '-' : stats?.completed_lessons || 0}</p>
                    ...
                    <div className="bg-card rounded-2xl border border-border/50 p-5 shadow-soft">
                        ...
                        <p className="text-2xl font-bold text-foreground">{isLoading ? '-' : `${stats?.avg_progress || 0}%`}</p>
                ...
                <section className="bg-card rounded-2xl border border-border/50 p-6 shadow-soft animate-fade-in">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h3 className="font-serif text-xl font-semibold text-foreground">Weekly Activity</h3>
                            <p className="text-xs text-muted-foreground">Number of lessons completed daily</p>
                        </div>
                ...
                <section className="bg-card rounded-2xl border border-border/50 p-6 shadow-soft animate-fade-in">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h3 className="font-serif text-xl font-semibold text-foreground">Learning Consistency</h3>
                            <p className="text-xs text-muted-foreground">Time spent meditating (minutes)</p>
                        </div>
                                ...
                                <AreaChart data={timeData}>
                                    <defs>
                                        <linearGradient id="colorMinutes" x1="0" y1="0" x2="0" y2="1">
                                            <stop offset="5%" stopColor="#4A5D4A" stopOpacity={0.3} />
                                            <stop offset="95%" stopColor="#4A5D4A" stopOpacity={0} />
                                        </linearGradient>
                                    </defs>
                                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(0,0,0,0.05)" />
                                    <XAxis
                ...
                <section className="animate-fade-in">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-serif text-xl font-semibold text-foreground">In Progress</h3>
                        <button
                    ...
                    <div className="space-y-3">
                        ...
                        ) : stats?.courses?.filter((c: any) => c.progress_percent < 100).length > 0 ? (
                            stats.courses
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                ...
                                                <span className="text-xs font-semibold text-foreground">{course.progress_percent || 0}%</span>
                                            </div>
                                        </div>
