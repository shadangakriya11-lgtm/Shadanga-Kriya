...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <main>
        {/* Hero Section */}
        <section className={`relative flex items-center ${isNativePlatform ? 'min-h-screen' : 'min-h-[90vh]'}`}>
          {/* Hero Background Gradient */}
          <div className="absolute inset-0 -z-10 brand-gradient-bg" />

          <div className={`container mx-auto px-6 ${isNativePlatform ? 'py-0' : 'py-20 md:py-32'}`}>
            {/* Native App - Premium Welcome Screen */}
            {isNativePlatform ? (
              <div className="flex flex-col items-center justify-center min-h-[calc(100vh-2rem)] py-8">
                {/* Floating Logo Section - Native App */}
                <div className="relative flex items-center justify-center mb-10 animate-fade-in-scale animation-fill-both">
                  {/* Subtle glow behind logo */}
                  <div className="absolute w-56 h-56 rounded-full bg-gradient-to-br from-teal-400/15 via-cyan-400/10 to-amber-400/15 blur-2xl animate-breathe-glow" />

                  {/* Main logo with continuous floating animation */}
                ...
                {/* Tagline */}
                <p className="text-sm text-primary font-medium mb-6 text-center animate-fade-in animation-fill-both animate-delay-100">
                  Audio Therapy Platform
                </p>

                {/* Description Card */}
                <div className="w-full max-w-sm px-4 py-5 rounded-2xl bg-card/60 backdrop-blur-sm border border-border/30 mb-8 animate-fade-in-up animation-fill-both animate-delay-200">
                  <p className="text-sm text-muted-foreground leading-relaxed text-center">
                    Experience the transformative power of scientific meditation through guided audio sessions.
                  </p>
                </div>

                {/* Buttons - Native Optimized */}
                <div className="flex flex-col gap-3 w-full max-w-sm animate-fade-in-up animation-fill-both animate-delay-300">
                  <Button
                    size="lg"
                  ...
                  <Button
                    variant="outline"
                    size="lg"
                    className="w-full h-14 text-base rounded-xl border-2 border-border hover:border-primary hover:text-primary bg-card/50"
                    onClick={() => navigate("/auth")}
                  >
              ...
              <>
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                  ...
                  {/* Right Content - Logo Showcase (Desktop only) */}
        ...
        {/* Web-only sections - Hidden on Native App */}
        {!isNativePlatform && (
          <>
            {/* Features Section */}
      ...
      {/* Footer - Also hidden on Native App */}
      {!isNativePlatform && (
